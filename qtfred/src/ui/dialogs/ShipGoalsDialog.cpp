...
namespace fred {
namespace dialogs {
ShipGoalsDialog::ShipGoalsDialog(QWidget* parent, EditorViewport* viewport, bool multi)
	: QDialog(parent), ui(new Ui::ShipGoalsDialog()), _model(new ShipGoalsDialogModel(this, viewport, multi)),
	  _viewport(viewport)
{
	ui->setupUi(this);
	...
	for (int i = 0; i < ED_MAX_GOALS; i++) {
		...
		connect(subsys[i], QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int index) {
			int data = subsys[i]->itemData(index).value<int>();
...
void ShipGoalsDialog::setShip(int ship)
{
	_model->setShip(ship);
}
	...
	for (int i = 0; i < ED_MAX_GOALS; i++) {
		...
		if (value < 1) {
			...
			if (mode == AI_GOAL_DESTROY_SUBSYSTEM) {
				...
				subsys[i]->setCurrentIndex(subsys[i]->findData(value));
