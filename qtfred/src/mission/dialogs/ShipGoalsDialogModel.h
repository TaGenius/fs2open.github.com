...
class ShipGoalsDialogModel : public AbstractDialogModel {
  private:
	int Ai_goal_list_size = sizeof(_editor->getAi_goal_list()) / sizeof(ai_goal_list);
	void initializeData();
  ...
  public:
	ShipGoalsDialogModel(QObject* parent, EditorViewport* viewport, bool multi);
	bool apply() override;
